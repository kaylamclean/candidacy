...
Another new variable is the \etmiss significance $\mathcal{S}$ {\color{red}TODO: Citation!} It is defined using a log-likelihood ratio to estimate how likely it is that the reconstructed \etmissvec, summed from all reconstructed objects, is consistent with the true \etmissvec according to:
...
\noindent {\color{red}TODO: Discuss the parameters in LS models, advantages over simplified models, ...}

And there are even more models \cite{Bauer:2017fsw} being discussed as potential new $t$-channel benchmarks. Discussions will need to be had with the LHC DM WG in order to determine the best model(s) to investigate.
